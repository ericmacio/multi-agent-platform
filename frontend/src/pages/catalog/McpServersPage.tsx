import { McpServerList } from '@/features/catalog/McpServerList';
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-6 py-6">
      <header className="flex flex-col gap-1">
        <h1 className="text-xl font-medium text-text-primary">MCP servers</h1>
        <p className="text-sm text-text-secondary">
          MCP integrations available for enabling on an agent.
        </p>
      </header>

      {query.isPending && (
        <Card padding="none" className="p-4">
          <div className="flex flex-col gap-3" data-testid="mcp-servers-loading">
      ...
      {query.isSuccess && <McpServerList items={query.data} />}
    </div>
  );
}
