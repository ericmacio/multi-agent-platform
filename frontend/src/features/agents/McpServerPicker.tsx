...
import { useMcpServers } from '@/features/catalog/api';
...
export function McpServerPicker({
  ...
  onChange,
  disabled = false,
}: McpServerPickerProps): JSX.Element {
  ...
  return (
    <Card padding="md" className="flex flex-col gap-3">
      <header className="flex items-center justify-between gap-2">
        ...
        <Badge variant="accent" data-testid="mcp-server-picker-count">{`${value.length} selected`}</Badge>
      </header>

      {servers.isPending && (
        <div className="flex flex-col gap-2" data-testid="mcp-server-picker-loading">
      ...
      {servers.isError && (
        <div className="flex flex-col items-start gap-2" role="alert">
          <p className="text-sm font-medium text-text-primary">
            {errorCopy[servers.error.code]?.title ?? errorCopy.__unknown__.title}
          </p>
          <Button variant="secondary" size="sm" onClick={() => void servers.refetch()}>
            Retry
          </Button>
        </div>
      )}
      ...
      {servers.isSuccess && servers.data.length === 0 && (
        <EmptyState
          ...
          description="Ask an administrator to configure MCP servers on the backend."
        />
      )}
          ...
          {filtered.length === 0 ? (
            ...
            <ul className="flex max-h-72 flex-col gap-1 overflow-auto">
              {filtered.map((server) => (
                ...
                >
                  <Checkbox
                    checked={selectedSet.has(server.name)}
                    disabled={disabled}
